...
# Parameters
# ────────────────────────────────────────────────────────────────
strike = 630                              # common strike for call & put
...
gamma_thresh = 0.0275                   # |Γ| trigger for γ‑state hedge
...
def run_gamma_state_hedge(df: pd.DataFrame, thresh: float) -> pd.DataFrame:
    """Enter hedge when |Γ| ≥ thresh, exit when |Γ| < thresh."""
    position = 0.0
    hedged = False
    tgt, qty, cf = [], [], []

    for g, d, S in zip(df.gamma_port, df.delta_port, df.S_last):
        if (not hedged) and abs(g) >= thresh:      # ENTER
            target = -d
            trade = target - position
            position = target
            hedged = True
        elif hedged and abs(g) < thresh:           # EXIT
            target = 0.0
            trade = -position
            position = 0.0
            hedged = False
    ...
    out = df.copy()
    out["target_shares"] = tgt
    out["trade_qty"] = qty
    out["cash_flow"] = cf
    out["cum_cash"] = out.cash_flow.cumsum()
    out["hedge_value"] = out.target_shares * out.S_last
    out["hedge_pnl_gamma"] = out.cum_cash + out.hedge_value
    out["pnl_gamma_total"] = out.pnl_port + out.hedge_pnl_gamma
    return out
...
df["hedge_pnl_first"]  = -(df.S_last - open_stock) * delta0
...
# γ-state hedge
# ────────────────────────────────────────────────────────────────
df = run_gamma_state_hedge(df, gamma_thresh)
...
print(f"Γ threshold (abs)             : {gamma_thresh}")
...
ax.plot(df.V_time, df.pnl_gamma_total,   label=f"γ‑state hedge (|Γ|≥{gamma_thresh})", lw=1.1, color="purple")
